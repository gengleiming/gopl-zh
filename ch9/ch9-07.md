...
func (memo *Memo) Get(key string) (value interface{}, err error) {
	...
	if e == nil {
		...
		close(e.ready) // broadcast ready condition  这里防止有其他请求过来时，判断e!=null时memo.f还没执行完会;保证了memo.f执行完，下一个请求才能返回
	} else {
	...
	return e.res.value, e.res.err  // 当memo.f调用完之后，会立即接收到通知，进行读操作
}
```
